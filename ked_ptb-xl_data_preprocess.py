...
"""
#处理 PTB-XL 数据集，加载数据、预处理标签、划分训练集和测试集，并保存处理后的数据。
def handler_data(experiment_name, task, datafolder, sampling_frequency = 100, min_samples = 0, train_fold = 8, val_fold = 9, test_fold = 10, folds_type = 'strat'):
    ...
    # Preprocess label data：(21799,29) 根据任务类型聚合标签
    labels = compute_label_aggregations(raw_labels, datafolder, task)

    # Select relevant data and convert to one-hot (21799,1000,12), (21799,30)   Y:(21388, 71)选择数据并转换为 one-hot 编码
    ...
    # Preprocess signal data预处理信号数据 对 ECG 信号数据进行标准化处理。
    ...
    #根据 strat_fold 列划分报告数据，并将训练集的报告数据保存到 CSV 文件。
...
def generate_ptb_label_gemini_augment():#主要任务是为 PTB-XL 数据集中的每个标签生成描述信息。这些描述信息是通过调用 Gemini API 生成的，最终保存到 ptbxl_label_map_description_gemini.json 文件中。
    ...
    generated_description_dict = {} ##初始化生成的描述字典
    for item in all_label_map.values():
        ##遍历 all_label_map 中的每个标签描述（item），调用 _generate_gemini_augment_ 函数生成描述。
        response = _generate_deepseek_augment_(item)
        print(response)
        generated_description_dict[item] = response##将生成的描述存储到 generated_description_dict 中。
    with open("ptbxl_label_map_description_gemini.json", "w") as f: ##将 generated_description_dict 保存到 ptbxl_label_map_description_gemini.json 文件中。
        json.dump(generated_description_dict, f)

def _generate_deepseek_augment_(item):
    ...
    client = OpenAI(api_key="sk-822d9523f40b48028f1b4a340e2423ea", base_url="https://api.deepseek.com")
